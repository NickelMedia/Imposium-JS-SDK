...
var ImposiumClient = /** @class */ (function () {
    ...
    function ImposiumClient(token, config) {
        ...
        if (ImposiumClient.config.trackingId && (this.idRegExp).test(ImposiumClient.config.trackingId)) {
            this.analytics = new Analytics_1.default(ImposiumClient.config.trackingId);
            this.pageView();
        ...
        if (ImposiumClient.config.video && this.analytics) {
            ImposiumClient.config.video.addEventListener('loadstart', function () { return _this.onLoad(); });
            ImposiumClient.config.video.addEventListener('play', function () { return _this.onPlay(); });
            ImposiumClient.config.video.addEventListener('pause', function () { return _this.onPause(); });
            ImposiumClient.config.video.addEventListener('ended', function () { return _this.onEnd(); });
        }
    ...
    ImposiumClient.prototype.checkProgress = function () {
        var v = ImposiumClient.config.video;
        if (v) {
            var perc = v.currentTime / v.duration, next = this.evts[this.lastEvtFired];
            if (perc > next) {
                this.analytics.send({
                    t: 'event',
                    ec: 'video_player',
        ...
        for (var inventoryId in inventory) {
            ...
            if (val && val.type === "file") {
                ...
                if (!name_1) {
                    ...
                    name_1 = 'inventory.' + type;
                }
                formData.append(inventoryId, val, name_1);
            }
        }
        // add the inventory
