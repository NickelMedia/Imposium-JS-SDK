...
class ImposiumClient {
    ...
    startEventProcessor(data, success, error) {
        const jobSpec = {
            'expId': data.expId,
            'actId': data.actId,
            'onSuccess': success,
        ...
        if (!this.messageConsumer) {
            this.messageConsumer = new MessageConsumer_1.MessageConsumer(jobSpec, this, this.api, stompConfig);
        }
        else {
...
class MessageConsumer {
     ...
     */
    constructor(job, delegate, api, config) {
        this.retried = 0;
        this.maxRetries = 3;
        this.job = job;
